...
# Connect to ExchangeOnline for the sole dumb purpose of getting access to the DistributionGroup
Connect-ExchangeOnline -UserPrincipalName $TokenUser

$userMailboxes = Get-DistributionGroupMember -Identity "Aztec-All"
if (!$userMailboxes)
{
...
{
    $counter = 1
    foreach ($userMailbox in $userMailboxes.PrimarySMTPAddress)
    {
        # Create a log file
        ...
        WriteToLogFile("Accessing "+$userMailbox+" "+$counter)
        ...
        $counter++
        WriteToLogFile("Finished syncing "+$userMailbox)
    }
}
